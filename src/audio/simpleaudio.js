...
var SimpleAudio = (() => { // eslint-disable-line no-unused-vars, no-var
		...
		function _playReturnsPromise() {
			...
			if (Has.audio) {
				try {
					...
					// NOTE (ca. January, 2020): Firefox will still log an "Autoplay is only allowed
					...
					//
					// QUESTION (ca. January, 2020): Keep this?  It's only here to appease Firefox,
	...
	*******************************************************************************/
	...
	class AudioTrack {
		...
		loop(loop) {
			...
			// Ensure the loop state has a sane value.
		...
		mute(mute) {
			...
			// Ensure the mute state has a sane value.
			this.#mute = !!mute;
		...
		#syncMute() {
			this.audio.muted = this.#mute || _masterMute;
		}
		...
		rate(rate) {
			...
			// Clamp the playback rate to a sane value—some browsers also do this to varying degrees.
			//
			...
			*/
			this.#rate = Math.clamp(rate, 0.2, 5); // clamp to 5× slower & faster
		...
		#syncRate() {
			/*
			const rate = this.#rate * _masterRate;
			this.audio.playbackRate = rate < 0
				? Math.clamp(rate, -0.2, -5) // clamp to 5× slower & faster, backward
				: Math.clamp(rate, 0.2, 5);  // clamp to 5× slower & faster, forward
			*/
			this.audio.playbackRate = Math.clamp(this.#rate * _masterRate, 0.2, 5); // clamp to 5× slower & faster
		}
		...
		time(time) {
			...
			//
			// NOTE (ca. December, 2018): Firefox will still log an `InvalidStateError` to the
		...
		volume(volume) {
			...
			// Clamp the volume to a sane value.
			this.#volume = Math.clamp(volume, 0, 1); // clamp to 0 (silent) & 1 (full loudness)
		...
		#syncVolume() {
			this.audio.volume = Math.clamp(this.#volume * _masterVolume, 0, 1); // clamp to 0 (silent) & 1 (full loudness)
		}
		...
		isPaused() {
			// If the selected audio resource is a stream, `currentTime` may return a
			// non-zero value even at the earliest available position within the stream
			// as the browser may have dropped the earliest chunks of buffered data or
			// the stream may have a timeline which does not start at zero.
			//
			// In an attempt to guard against these possibilities, as best as we can,
			// we test `duration` against `Infinity` first, which should yield true for
			// actual streams.
		...
		#create(sourceList) {
			...
			//
			// Use of the DOM factory method, `document.createElement('audio')`, should
			// be preferred over use of the constructor, `new Audio()`.  The reason being
			// that objects created by the latter are, erroneously, treated differently,
			// often unfavorably, by certain browser engines—e.g., within some versions
			// of the iOS browser core.
			//
			// Notably, the only difference between the two, per the specification, is
			// that objects created via the constructor should have their `preload`
			// property automatically set to 'auto'.  Thus, there's no technical reason
			// to prefer usage of the constructor, even discounting browser quirks.
			const audio = document.createElement('audio');
			...
			this.#finalize(audio, usedSources, clone(sourceList));
		}
		...
		#finalize(audio, sources, originals) {
			// Set up our own public properties.
			...
			// Set up our own private properties.
			this.#error   = false;
			this.#faderId = null;
			this.#mute    = false;
			this.#rate    = 1;
			...
			jQuery(this.audio)
				// Upon receiving a `loadstart` event on the audio element, set
				// `#error` to `false`.
				.on('loadstart.AudioTrack', () => this.#error = false)

				// Upon receiving an `error` event on the audio element, set `#error`
				// to `true`.
				//
				...
				// trigger an `error` event on the audio element—that being necessary
				// because the source `error` event does not bubble.
				.find('source:last-of-type')
				.on('error.AudioTrack', () => this.#trigger('error'));
			...
			subscribe(this, mesg => {
				...
				switch (mesg) {
					...
					case 'mute':   this.#syncMute();   break;
					case 'rate':   this.#syncRate();   break;
					case 'stop':   this.stop();        break;
					case 'volume': this.#syncVolume(); break;
	...
	*******************************************************************************/
	...
	class AudioList {
		...
		queue;   // The currently queued tracks.
		...
		destroy() {
			// NOTE: Strictly speaking, self-destruction is not necessary as this
			// object will, eventually, be garbage collected.
		...
		fade(duration, toVol, fromVol) {
			...
			this.#volume = toVol; // NOTE: A kludge, but necessary.
		...
		mute(mute) {
			...
			if (this.current !== null) {
				this.current.track.mute(this.#mute);
			}
		...
		shuffle(shuffle) {
			...
			if (this.queue.length > 0) {
				...
				if (!this.#shuffle && this.current !== null && this.queue.length > 1) {
		...
		#create(trackList) {
			...
			this.#finalize(trackList.map(trackObj => {
				...
				track.rate(rate);
				track.volume(volume);
				track.on('ended.AudioList', () => this.#onEnd());
		...
		#copy(obj) {
			this.#finalize(clone(obj.tracks));
		}
	...
	*******************************************************************************/
	...
	class AudioRunner {
		...
		unload() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.unload);
		}

		play() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.play);
		}

		playWhenAllowed() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.playWhenAllowed);
		}

		pause() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.pause);
		}

		stop() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.stop);
		}
		...
		fade(duration, toVol, fromVol) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.fade, duration, toVol, fromVol);
		}
		...
		fadeIn(duration, fromVol) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.fadeIn, duration, fromVol);
		}
		...
		fadeOut(duration, fromVol) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.fadeOut, duration, fromVol);
		}

		fadeStop() {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.fadeStop);
		}
		...
		loop(loop) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.loop, loop);
			return this;
		}
		...
		mute(mute) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.mute, mute);
			return this;
		}
		...
		rate(rate) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.rate, rate);
			return this;
		}
		...
		time(time) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.time, time);
			return this;
		}
		...
		volume(volume) {
			...
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.volume, volume);
			return this;
		}

		on(...args) {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.on, ...args);
			return this;
		}

		one(...args) {
			AudioRunner.#run(this.trackIds, AudioTrack.prototype.one, ...args);
			return this;
		}
	...
	*/
	...
	function listClear() {
		_lists.forEach(list => list.destroy());
		_lists.clear();
	}
